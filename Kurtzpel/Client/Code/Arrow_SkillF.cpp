...
CArrow_SkillF::CArrow_SkillF(LPDIRECT3DDEVICE9 pGraphicDev)
	: CUnit(pGraphicDev)
{
	m_sStat.m_fSpeed = 120.f;
	m_LifeTime = 3.f;
}

CArrow_SkillF::~CArrow_SkillF(void)
{

}

HRESULT Client::CArrow_SkillF::Add_Component(void)
{
	...
	m_sComponent.m_pTransformCom->m_vScale = { 0.03f, 0.03f, 0.03f };

	return S_OK;
}


CArrow_SkillF* CArrow_SkillF::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CArrow_SkillF*	pInstance = new CArrow_SkillF(pGraphicDev);
...
void Client::CArrow_SkillF::Render_Object(void)
{
...
HRESULT Client::CArrow_SkillF::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
void Client::CArrow_SkillF::Collision(CSphereCollider* _mySphere, CUnit* _col, CSphereCollider* _colSphere, const _float& fTimeDelta)
{
...
void Client::CArrow_SkillF::Create_Coll()
{
	CSphereCollider* sphereCol = CSphereCollider::Create(m_pGraphicDev);
	sphereCol->Set_Transform(3.f);
...
void Client::CArrow_SkillF::Set_SpeedToLife(float _speed, float _life)
{
