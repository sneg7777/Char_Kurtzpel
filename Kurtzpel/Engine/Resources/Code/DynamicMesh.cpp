...
_bool Engine::CDynamicMesh::Is_AnimationSetEnd_Percent(float per)
{
	return m_pAniCtrl->Is_AnimationSetEnd_Percent(per);
}

_bool Engine::CDynamicMesh::Is_AnimationSetEnd_Check()
{
	return m_pAniCtrl->Is_AnimationSetEnd_Check();
}

void Engine::CDynamicMesh::Is_Loop_Change(_bool change)
{
	m_pAniCtrl->m_Loop = change;
	return;
}
...
void CDynamicMesh::Play_Animation(const _float& fTimeDelta)
{
	...
	_matrix		matTemp;
	Update_FrameMatrices((D3DXFRAME_DERIVED*)m_pRootFrame, D3DXMatrixRotationY(&matTemp, D3DXToRadian(m_Angle)));
}
