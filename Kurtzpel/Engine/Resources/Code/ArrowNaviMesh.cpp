#include "ArrowNaviMesh.h"
USING(Engine)

Engine::CArrowNaviMesh::CArrowNaviMesh(LPDIRECT3DDEVICE9 pGraphicDev)
	: CMesh(pGraphicDev)
	, m_dwIndex(0)
{
...
Engine::CArrowNaviMesh::CArrowNaviMesh(const CArrowNaviMesh& rhs)
	: CMesh(rhs)
...
Engine::CArrowNaviMesh::~CArrowNaviMesh(void)
{
...
void CArrowNaviMesh::Render_NaviMeshes(void)
{
	for (auto& iter : m_vecCell)
		iter->Render_Cell();
}

_vec3 CArrowNaviMesh::Move_OnNaviMesh(const _vec3* pTargetPos, const _vec3* pTargetDir, _ulong* _index, bool* _check)
{
...
HRESULT Engine::CArrowNaviMesh::Link_Cell(void)
{
...
Engine::CComponent* Engine::CArrowNaviMesh::Clone(void)
{
	return new CArrowNaviMesh(*this);
}

void Engine::CArrowNaviMesh::Free(void)
{
