...
Engine::CCell::MOVING Engine::CCell::CompareCell2(_vec3* pEndPos, _ulong* pCellIndex, _vec3* playerDir)
{
	for (_ulong i = 0; i < LINE_END; ++i)
	{
		if (CLine::COMPARE_LEFT == m_pLine[i]->Compare(&_vec2(pEndPos->x, pEndPos->z)))
		{
			if (nullptr == m_pNeighbor[i]) {
				/*_vec2 line_Start = m_pLine[i]->Get_vPoint(CLine::POINT_START);
				_vec2 line_Finish = m_pLine[i]->Get_vPoint(CLine::POINT_FINISH);
				_vec2 min, max;
				if (line_Start.x > line_Finish.x) {
					min.x = line_Finish.x; max.x = line_Start.x;
				}
				else {
					max.x = line_Finish.x; min.x = line_Start.x;
				}
				if (line_Start.y > line_Finish.y) {
					min.y = line_Finish.y; max.y = line_Start.y;
				}
				else {
					max.y = line_Finish.y; min.y = line_Start.y;
				}
				if (pEndPos->x > max.x || pEndPos->x < min.x || pEndPos->z > max.y || pEndPos->z < min.y) {
					//playerDir->x = 0.f;
					//playerDir->z = 0.f;
					return CCell::STOP;
				}*/

				_vec3 joinDir = { playerDir->z * -1.f, 0.f, playerDir->x };
				D3DXVec3Normalize(&joinDir, &joinDir);
				//_vec3 joinDir = *playerDir;
				_vec3 LineDir = { m_pLine[i]->Get_vNormal().x, 0.f, m_pLine[i]->Get_vNormal().y };
				
				_vec3 TempDir;
				
				TempDir.x = joinDir.x - (LineDir.x * (joinDir.x * LineDir.x));
				TempDir.y = 0.f;
				TempDir.z = joinDir.z - (LineDir.z * (joinDir.z * LineDir.z));
				D3DXVec3Normalize(&TempDir, &TempDir);

				playerDir->x = TempDir.x;
				playerDir->z = TempDir.z;
				return CCell::STOP;
				//return CCell::STOP;
			}
			else
